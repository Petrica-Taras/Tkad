...
import Tkinter, tkMessageBox, functools, os
...
class MenuToolbar(Tkinter.Menu):
    ...
    def createMenuTree(self):
        ...
        try:
            ...
            tkMessageBox.showerror("Damaged Menu Configuration File", "The menu configuration file have been tampered with it. Will abort operation ...")
            self.master.quit()
            self.master.destroy()
            exit(1)
        ...
        def recursMenuTree(node):
            ...
            for j in menulist:
                if j.nodeName == "MenuItem": 
                    node[menulist.index(j)]=[j.attributes["name"].value, j.attributes.items()]
                else: # must by a MenuSubDir
                    node[menulist.index(j)]=[j.attributes["name"].value, j.attributes.items(), j]
                ...
                if len(j) == 2: # simple item j[0] is the label of the menu item; j[1] - various options
                    ...
                    if j[0] == "Separator" and j[1][0][1] == "Separator":
                        ...
                        try:
                            ...
                            menuname[0].add_command(label=attr["name"], compound=Tkinter.LEFT, image=self.img[-1], command=functools.partial(eval(attr["callback"]), self.master), accelerator=attr["accelerator"])
                        except:
                            menuname[0].add_command(label=attr["name"], compound=Tkinter.LEFT, image="", command=functools.partial(eval(attr["callback"]), self.master), accelerator=attr["accelerator"])
                        del attr
